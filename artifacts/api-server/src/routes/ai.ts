...
router.post("/tts-generate", requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Sesin GERÇEK süresini sunucuda ölçüyoruz. Bu kritik: önceden istemci
    // tarafı, "sesin bitmesi" anını kendi tarayıcısının ses çalma olayına
    // (onended) göre belirliyordu — yavaş/dalgalı internetli bir katılımcıda
    // bu, aynı sesin diğerlerinden çok daha GEÇ bitmesine yol açabiliyordu
    // (arabelleğe alma gecikmesi). Bu da uzun (örn. 45sn) seslerde bazı
    // katılımcıların soruyu görüp cevaplayamadan süresinin bitmesine sebep
    // oluyordu. Artık sunucu sesin gerçek süresini ölçüp bunu TÜM
    // istemcilere aynı sabit değer olarak gönderiyor; her istemci "soru
    // metnini göster" anını kendi ses oynatma olayından değil, bu ortak
    // süreden hesaplıyor — böylece herkes aynı anda, adil bir cevap
    // penceresine geçiyor.
    let durationSec = 0;
    try {
      const meta = await parseBuffer(audioBuffer, "audio/mpeg");
      durationSec = Math.ceil(meta.format.duration || 0);
    } catch (e) {
      logger.warn({ err: e }, "Ses süresi ölçülemedi, istemci taraflı geri dönüş kullanılacak");
    }
    ...
    res.json({ audioData: `data:audio/mpeg;base64,${base64}`, durationSec });
    return;
  } catch (e) {

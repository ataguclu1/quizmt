...
export function setupSocketIO(httpServer: HttpServer) {
  const io = new SocketIOServer(httpServer, {
    ...
    // KRİTİK DÜZELTME: Önceden pingTimeout 60sn + pingInterval 25sn idi —
    // bu, bir bağlantı TAMAMEN kopmadan ama paket kaybı/sıkışık wifi
    // yüzünden "sessizce bozulduğunda" (soket teknik olarak hâlâ "bağlı"
    // görünüyor ama yayınlar ulaşmıyor), sistemin bunu fark edip yeniden
    // bağlanmaya çalışmadan önce ~85 saniye geçebiliyordu — bu süre
    // boyunca o katılımcının ekranı "donmuş" görünüyordu, ancak host
    // sıradaki soruya geçip yeni bir yayın tetiklenince (ya da 85sn dolup
    // otomatik yeniden bağlanma başlayınca) düzeliyordu. Şimdi çok daha
    // sık ping atıp çok daha hızlı zaman aşımına uğratıyoruz — kötü bir
    // bağlantı en fazla ~30 saniyede fark edilip yeniden bağlanma
    // sürecine sokuluyor. Normal, kısa süreli ağ titremelerini yanlışlıkla
    // "kopma" saymayacak kadar toleranslı, ama kalabalık sınıf wifi'sinde
    // gerçek bir tıkanıklığı çok daha hızlı tespit edecek kadar sıkı.
    pingTimeout: 20_000,
    pingInterval: 10_000,
  ...
  io.on("connection", (socket) => {
    ...
    // KRİTİK DÜZELTME: Yukarıdaki start-game/show-question/next-question
    // handler'ları qStartTs'i YAYIN ANINDA (broadcast) ayarlıyor — ama
    // istemcilerde asıl geri sayım hemen başlamıyor: önce 3-2-1 animasyonu
    // (~2.4sn), sesli sorularda ayrıca ikon+ses süresi, x2 puanlı sorularda
    // da bonus bildirimi araya giriyor. Bu yüzden qStartTs, gerçek geri
    // sayımın başladığı andan HER ZAMAN biraz erken kalıyordu. Sonuç: biri
    // F5 atıp yeniden bağlandığında, sunucu "elapsedSec"i olması gerekenden
    // FAZLA hesaplıyor, o kişiye gerçekte kalan süreden DAHA AZ bir süre
    // gösteriliyordu — host'un/diğerlerinin ekranında görünenle uyuşmuyordu.
    // Çözüm: Host'un kendi ekranında geri sayım GERÇEKTEN başladığı anda
    // (tüm animasyon/ses gecikmeleri bittikten sonra) bu event'i gönderiyor,
    // sunucu da qStartTs'i o TAM ana göre güncelliyor — artık "host'ta kaç
    // saniye kaldıysa" hesaplaması, gerçek zamana birebir dayanıyor.
    socket.on("question-timer-started", (data: { pin: string; qIdx: number }) => {
      const session = sessions.get(data.pin);
      if (!session || session.hostSocketId !== socket.id) return;
      if (data.qIdx !== session.qIdx) return; // host geç kalmış bir sinyal göndermiş olabilir, geçersiz kıl
      session.qStartTs = Date.now();
    });
